...
class Game {
    ...
    bindEvents() {
        ...
        // 教程按钮：显示所有夜晚的教程
        this.tutorialBtn.addEventListener('click', () => this.showAllTutorials());
    ...
    // 触控旋转按钮：按下/长按按对应方向旋转视角
    initRotationButtons() {
        const leftBtn = document.getElementById('rotate-left-btn');
        const rightBtn = document.getElementById('rotate-right-btn');
        if (!leftBtn || !rightBtn) return;

        // 在按钮上设置旋转标志，RAF 循环会据此移动视角
        const startRotate = (btn, direction) => {
            if (!this.state.isGameRunning || this.state.cameraOpen || this.state.controlPanelOpen) return;
        ...
        const stopRotate = (btn) => {
            btn.classList.remove('active');
            if (btn === leftBtn) this.isRotatingLeft = false;
            if (btn === rightBtn) this.isRotatingRight = false;
        };

        // 使用 pointer events，并对该按钮捕获指针、阻止冒泡，
        // 避免 InputHandler 的 mousemove/touch 处理干扰或全局 touchend 清零
        [leftBtn, rightBtn].forEach((btn) => {
            const direction = btn === leftBtn ? 'left' : 'right';

            btn.addEventListener('pointerdown', (e) => {
                e.preventDefault();
                e.stopPropagation();
                try { btn.setPointerCapture(e.pointerId); } catch (_) {}
                startRotate(btn, direction);
            });
            btn.addEventListener('pointerup', (e) => {
                e.stopPropagation();
                stopRotate(btn);
            });
            btn.addEventListener('pointercancel', () => stopRotate(btn));
            // flag-down 时为 `setPointerCapture` 模拟 leave；额外保留
            btn.addEventListener('pointerleave', () => stopRotate(btn));

            // 兼容不支持的 pointer events 的回退
            btn.addEventListener('mousedown', (e) => {
                e.preventDefault();
                e.stopPropagation();
                startRotate(btn, direction);
            });
            const mouseStop = () => stopRotate(btn);
            btn.addEventListener('mouseup', mouseStop);
            btn.addEventListener('mouseleave', mouseStop);
        });
    }

    // 初始化右上角触控开关：切换左/右旋转按钮的显示
    initTouchToggle() {
        const toggleBtn = document.getElementById('touch-toggle-btn');
        const gameScreen = document.getElementById('game-screen');
        if (!toggleBtn || !gameScreen) return;

        // 默认开启触控旋转按钮
        toggleBtn.classList.add('touch-on');
        gameScreen.classList.remove('touch-controls-off');

        toggleBtn.addEventListener('click', (e) => {
            e.stopPropagation();
            const off = gameScreen.classList.toggle('touch-controls-off');
            toggleBtn.classList.toggle('touch-on', !off);
            // 关闭触控时停止正在进行的旋转
            if (off) {
                this.isRotatingLeft = false;
                this.isRotatingRight = false;
    ...
    showAllTutorials() {
        ...
        tutorialContent.innerHTML = `
            ...
            <h3>如何防御史蒂芬·霍金（第 3 夜）</h3>
            <p>
                史蒂芬·霍金始终待在 CAM 6，且不受声音诱饵的影响。
                你需要每隔一段时间电击霍金，以防止他离开 CAM 6。
            </p>
            <p class="tutorial-hint">建议截图保存，方便游戏中随时查看。</p>
            <button id="tutorial-got-it">明白了</button>
        `;
        // 重新绑定按钮事件（innerHTML 替换后需重新绑定）
